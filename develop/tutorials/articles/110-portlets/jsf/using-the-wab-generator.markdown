...
Liferay supports the use of pre-configured WABs (ones that have been configured
by the developer at build-time) and also those that have been auto-generated by
the WAB Generator at deploy time. The WAB Generator transforms a general
WAR-style portlet into a WAB during its deployment process. So what exactly does
the WAB Generator do to a WAR file to transform it into a WAB?

Consider the following **project source** folder structure of a WAR-style portlet:
...
When a WAR-style portlet is built by a build-time system like the maven-war-plugin,
the portlet's folder structure is transformed to something like this
...
During deploy time, the Liferay WAB Generator will auto-configure an OSGi-ready
`MANIFEST.MF` file. If you want to affect the content of the manifest file, you
can place BND directives and OSGi headers directly into the
`WEB-INF/liferay-plugin-package.properties` file. A `bnd.bnd` and/or a build-time
plugin (e.g., `bnd-maven-plugin`) should not be provided for your WAR plugin,
